...
local lookup = {
  ...
  dbRecent = 'DBRecent', -- test dbrecent
...
function setFilter:Filter(slotData)
   -- logItem(slotData)
    ...
    if (price ~= nil) then
      ...
      -- test 20000g
      if (min == 'LTwentyFiveThousand') then return nil end

      if (price > 200000000) then
        return '20k+'
      end
      -- test 15000g
      if (min == 'ITwentyThousand') then return nil end

      if (price > 150000000) then
        return '15k+'
      end

      if (min == 'HFifteenThousand') then return nil end

      if (price > 100000000) then
        return '10k+'
      end 

      if (min == 'GTenThousand') then return nil end

      if (price > 50000000) then
        return '5k+'
      end 
      -- test 2500g
      if (min == 'FiveThousand') then return nil end

      if (price > 25000000) then
        return '2.5k+'
      end

      if (min == 'ETwoThousandFiveHundred') then return nil end 

      if (price > 10000000) then
        return '1k+'
      end 
      -- test 750g
	    if (min == 'DOneThousand') then return nil end
	  
	    if (price > 7500000) then
		    return '750+'
	    end

      if (min == 'CSevenFifty') then return nil end 

      if (price > 5000000) then
        return '500+'
      end
	  
      if (min == 'BFiveHundred') then return nil end 
  ...
  return {
    ...
    minGroup = {
      ...
      values = {
        ATwoFifty = '250g',
        BFiveHundred = '500g',
        CSevenFifty = '750g', -- test 750g
        DOneThousand = '1000g',
        ETwoThousandFiveHundred = '2500g', -- test 2500g
        FFiveThousand = '5000g',
        GTenThousand = '10000g',
        HFifteenThousand = '15000g', -- test 15000g
        ITwentyThousand = '20000g', -- test 20000g
        LTwentyFiveThousand = '25000g'
      }
    },
    versioninfo = {
      name = L['Version: 1.7'],
